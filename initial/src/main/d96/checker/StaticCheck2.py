...
class bigProgram:
    ...
    def getClassSymbol(self, classname):
        a = list(filter(lambda x: x.name == classname and type(x.mtype) == Ctype, self.arr))
        if len(a) == 0:
            return False
        return a[0]
...
class StaticChecker(BaseVisitor):
    ...
    def visitMethodDecl(self,ast: MethodDecl, c_localBound):
        ...
        kind = ast.kind
        c.append(Symbol(name, mtype, isClassMember=True, kind=kind))
        localBound = len(c)
